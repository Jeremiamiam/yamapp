...
type TimelineEntry =
  | { kind: 'orchestrator'; text: string; id: number }
  | { kind: 'handoff'; to: AgentId; reason: string; id: number }
  | { kind: 'agent'; agent: AgentId; text: string; done: boolean; id: number }
  | { kind: 'report'; text: string; id: number };
...
export default function CreativeBoardPage() {
  ...
  const addEntry = (entry: { kind: 'orchestrator'; text: string } | { kind: 'handoff'; to: AgentId; reason: string } | { kind: 'agent'; agent: AgentId; text: string; done: boolean } | { kind: 'report'; text: string }) => {
    setTimeline((prev) => [...prev, { ...entry, id: nextId() } as TimelineEntry]);
  };

  const updateLastAgent = (agent: AgentId, chunk: string) => {
    setTimeline((prev) => {
      const copy = [...prev];
  ...
  const markAgentDone = (agent: AgentId) => {
    setTimeline((prev) => {
      const copy = [...prev];
      for (let i = copy.length - 1; i >= 0; i--) {
        if (copy[i].kind === 'agent' && (copy[i] as { agent: AgentId }).agent === agent) {
      ...
      <div style={{ maxWidth: 720, margin: '0 auto', padding: '48px 24px' }}>
        ...
        <div style={{ marginBottom: 48 }}>
          ...
          <p style={{ fontSize: 14, color: 'var(--text-muted)', marginTop: 8, marginBottom: 0 }}>
            3 agents en cascade — Stratège → Copywriter → Devil's Advocate
          </p>
        </div>
        ...
        <div style={{ marginBottom: 32 }}>
          ...
          <div style={{ display: 'flex', justifyContent: 'flex-end', marginTop: 12 }}>
            <button
              ...
              style={{
                background: running ? 'var(--bg-tertiary)' : 'var(--accent-lime)',
            ...
            >
              {running ? 'Board en cours…' : 'Lancer le Board →'}
            </button>
          </div>
        </div>
          ...
          <div>
            <div style={{
              ...
              <div style={{
                ...
                height: 6,
                borderRadius: '50%',
                background: running ? 'var(--accent-lime)' : 'var(--text-muted)',
                animation: running ? 'pulse 1.2s ease-in-out infinite' : 'none',
              }} />
              {running ? 'Session en cours' : 'Session terminée'}
            </div>

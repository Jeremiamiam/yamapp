...
export function DocumentsSection({ clientId }: DocumentsSectionProps) {
  ...
  return (
    <div className="bg-[var(--bg-card)] rounded-xl sm:rounded-2xl border border-[var(--border-subtle)] overflow-hidden">
      <div className="px-4 sm:px-5 py-4 border-b border-[var(--border-subtle)] flex items-center gap-2 flex-wrap">
        <FileText />
        ...
        <div className="ml-auto flex items-center gap-1">
          <button
            ...
            title="Report Plaud (upload & synthèse)"
          >
            <PlaudLogo className="w-4 h-4" />
          </button>
          <button
            onClick={() => openDocumentModal(clientId)}
            className="p-1.5 rounded-lg bg-[var(--accent-amber)]/10 text-[var(--accent-amber)] hover:bg-[var(--accent-amber)]/20 transition-colors"
            title="Ajouter un document (brief, note…)"
          >
            <Plus />
          </button>
        </div>

...
## ✅ Déjà fait (état au 2026-02-14)

- **1.1** ModalManager centralisé (`src/components/ModalManager.tsx`) ✅
- **1.2** page.tsx nettoyé (ModalManager utilisé) ✅
- **1.3–1.5** ClientDetail décomposé en 5 sections (ContactsSection, DocumentsSection, LinksSection, ActivitySection, DeliverablesSection), ClientDetail < 150 lignes ✅
- **2.1** Sélecteurs store : getFilteredDeliverables(), getFilteredCalls() ✅
- **2.2** Timeline : useMemo / useCallback / memo(TimelineCard) déjà en place ✅
- **2.3** date-utils.ts créé (formatDate, formatTime, formatDocDate, isSameDay, isPast, isToday, etc.) ✅
- **2.4** styles.ts créé (STATUS_STYLES, CATEGORY_STYLES, DOCUMENT_TYPE_STYLES) ✅
- **3.1–3.3** react-hook-form + zod + validation.ts ; ContactForm, DocumentForm, CallForm, DeliverableForm refactorisés ✅
- **3.5** error-handler.ts + try/catch sur actions CRUD du store ✅
- **Icons** : non fait (icônes restent inline ; peut être fait plus tard si besoin)
